<template>
  <Card>
    ...
    <template #content>
      ...
      <div v-if="props.image" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <BaseCommonImage
          :src="props.image.fullUrl"
...
const { upload, loading } = useFileUpload({
  method: 'POST',
  field: 'image',
  path: `/api/v1/booking-providers/${route.params.bookingProviderId}/cover-image/upload`,
});
